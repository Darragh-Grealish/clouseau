...
// TEST: GIVEN an ArrayList WHEN remove is called on an empty list THEN it throws an exception
...
// TEST: GIVEN an ArrayList with elements WHEN contains is called for removed elements THEN it returns false
...
// TEST: GIVEN an ArrayList WHEN adding more elements than initial capacity THEN it resizes correctly
TEST(ArrayListTest, ResizeOnAdd) {
    ...
    EXPECT_EQ(list.get(14), 14); // Check last added element
}
...
// TEST: GIVEN an ArrayList WHEN get is called with an out-of-bounds index THEN it throws an exception
TEST(ArrayListTest, GetOutOfBounds) {
    ArrayList<int> list;
    list.add(1);
    EXPECT_THROW(list.get(1), std::out_of_range); // Out of bounds
    EXPECT_THROW(list.get(-1), std::out_of_range); // Negative index
}

// TEST: GIVEN an ArrayList WHEN an element is removed and then added again THEN it behaves as expected
...
TEST(ArrayListTest, CopyConstructor) {
    ...
    list1.add(1);
    list1.add(2);
    ArrayList<int> list2(list1); // Copy constructor
    list1.remove(0); // Remove element from list1
    EXPECT_EQ(list1.size(), 1);
    EXPECT_EQ(list2.size(), 2); // list2 should not be affected
    EXPECT_EQ(list2.get(0), 1); // Ensure copy is unaffected
    EXPECT_EQ(list2.get(1), 2); // Ensure copy retains its data
}

// TEST: GIVEN two ArrayLists WHEN one is moved THEN the original is in a valid but empty state
TEST(ArrayListTest, MoveConstructor) {
    ArrayList<int> list1;
    list1.add(1);
    list1.add(2);
    ArrayList<int> list2(std::move(list1)); // Move constructor
    EXPECT_EQ(list2.size(), 2);
    EXPECT_EQ(list1.size(), 0); // list1 should be empty after move
}

// TEST: GIVEN an ArrayList with multiple elements WHEN removing elements in sequence THEN the list updates correctly
TEST(ArrayListTest, MultipleRemovals) {
    ...
    EXPECT_EQ(list.get(0), 1); // New first element
    list.remove(2); // Remove element '3'
    EXPECT_EQ(list.size(), 3);
    EXPECT_EQ(list.get(2), 4); // New last element
}
